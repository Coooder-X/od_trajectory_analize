        ...
        with h5py.File(data_source_path, 'r') as f:
            ...
            for i in range(0, len(f['trips'])):
                ...
                if needTime:  # false, why?
                    ...
                    # sql = "INSERT INTO trj_table (NULL, month, date, start_time, end_time, point_list) VALUES (%d, %d, %d, %d, %s)"
...
#     if check_table_exists(db_name, 'od_table'):
#         print(f'数据库 {db_name} 中，表 od_table 已存在')
#         return
#     print(f'数据库 {db_name} 中，表 od_table 正在创建...')
#     db = get_db_connection()
#     mycursor = db.cursor()
#     mycursor.execute('CREATE TABLE od_table (' +
#                      'id' +
#                      'lon' +
#                      'lat' +
#                      'month' +
