...
from make_origin_graph import get_origin_graph
...
def build_my_graph(cluster_res_dict):
    ...
    for point in cluster_res_dict:
        ...
        if cluster not in cluster_point_dict:
            ...
            cluster_point_dict[cluster].sort()
    ...
    with open(f'../od_flow_dict.pkl', 'rb') as f:
        od_flow_dict = pickle.loads(f.read())['od_flow_dict']
    g, filtered_adj_dict = get_origin_graph(od_region, out_adj_dict, od_flow_dict)
    con = avg_CON(g.G, cluster_point_dict, cluster_res_dict, False)
    print('CON =', con)
    print('cluster_point_dict', cluster_point_dict)
    return cluster_point_dict
...
def cluster(graph, used_points, X, num_cluster, true_clusters, alpha=0.2, beta=0.35, t=15, tmax=200, ri=False):
    ...
    M = csc_matrix((np.ones(labels.shape[1]), (np.arange(0, M.shape[0]), labels.flatten())), shape=(M.shape))
    M = M.todense()
...
def draw_grid_cluster(cluster_point_dict):
    import matplotlib.pyplot as plt
    import numpy as np
    # 创建一个 10x10 的矩形网格
    grid = np.zeros((10, 10))
    # 将字典中的值填充到网格中
    for key, values in cluster_point_dict.items():
        for value in values:
            x = value // 10
            y = value % 10
            grid[x, y] = key
    # 定义颜色映射
    cmap = plt.cm.get_cmap('tab10', len(cluster_point_dict))
    # 绘制矩形网格
    plt.figure(figsize=(6, 6))
    plt.imshow(grid, cmap=cmap, origin='lower')
    # 添加颜色条
    cbar = plt.colorbar(ticks=range(len(cluster_point_dict)))
    cbar.ax.set_yticklabels([str(key) for key in cluster_point_dict.keys()])
    # 添加网格线
    # plt.grid(which='major', color='black', linestyle='-', linewidth=2)
    # 添加网格线
    plt.grid(which='both', color='black', linestyle='-', linewidth=0.5)
    plt.xticks(np.arange(-0.5, 9.5, 1), np.arange(0, 10, 1))
    plt.yticks(np.arange(-0.5, 9.5, 1), np.arange(0, 10, 1))
    # 显示图像
    plt.show()
...
if __name__ == '__main__':
    ...
    cluster_point_dict = build_my_graph(res_dict)
    draw_grid_cluster(cluster_point_dict)
